...
Scene Scene::shapeScene() {
	...
	shared_ptr<Material> green = make_shared<Material>(vec3(0, 0.8f, 0), 1.05f, 0.1f, 0);
	shared_ptr<Material> texture = make_shared<Material>(vec3(0.5), 1.05f, 0.1f, 0);
	texture->addTexture(CGRA_WORKDIR + string("res/textures/checkerboard.jpg"));
	//objects.push_back(make_shared<SceneObject>(make_shared<AABB>(vec3(-3, 0, -5), vec3(0.5)), texture));

objects.push_back(make_shared<SceneObject>(make_shared<Sphere>(vec3(1, 1, 0), 2), texture));

	// YOUR CODE GOES HERE
	// ...

	//objects.push_back(make_shared<SceneObject>(make_shared<Plane>(vec3(0, -1, 0), vec3(0,1,0)), texture));

	//objects.push_back(make_shared<SceneObject>(make_shared<Disc>(vec3(1, 0, -5), vec3(0, 0, 1), 1), texture));

	//objects.push_back(make_shared<SceneObject>(make_shared<Triangle>(vec3(-10, 0, -5), vec3(1, 9, 6), vec3(-5, 4, 2)), texture));
